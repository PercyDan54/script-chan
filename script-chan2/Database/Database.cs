...
namespace script_chan2.Database
{
    public static class Database
    {
        ...
        private static void InitTournaments()
        {
            ...
            using (var conn = GetConnection())
            using (var command = new SQLiteCommand("SELECT id, name, gameMode, teamMode, winCondition, acronym, teamSize, roomSize, pointsForSecondBan, allPicksFreemod, allPicksNofail, mpTimerCommand, mpTimerAfterGame, mpTimerAfterPick, welcomeString, brInitialLivesAmount FROM Tournaments", conn))
            using (var reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    ...
                    var allPicksNofail = Convert.ToBoolean(reader["allPicksNofail"]);
        ...
        public static int AddTournament(Tournament tournament)
        {
            ...
            using (var conn = GetConnection())
            {
                using (var command = new SQLiteCommand(@"INSERT INTO Tournaments (name, gameMode, teamMode, winCondition, acronym, teamSize, roomSize, pointsForSecondBan, allPicksFreemod, allPicksNofail, mpTimerCommand, mpTimerAfterGame, mpTimerAfterPick, welcomeString, brInitialLivesAmount)
                VALUES (@name, @gameMode, @teamMode, @winCondition, @acronym, @teamSize, @roomSize, @pointsForSecondBan, @allPicksFreemod, @allPicksNofail, @mpTimerCommand, @mpTimerAfterGame, @mpTimerAfterPick, @welcomeString, @brInitialLivesAmount)", conn))
                {
                    ...
                    command.Parameters.AddWithValue("@allPicksNofail", tournament.AllPicksNofail);
        ...
        public static async Task InitMatches()
        {
            ...
            using (var conn = GetConnection())
            using (var command = new SQLiteCommand(@"SELECT id, tournament, mappool, name, roomId, gameMode, teamMode, winCondition, teamBlue, teamBluePoints, teamBlueCoins, teamRed, teamRedPoints, teamRedCoins, teamSize, roomSize,
            ...
            using (var reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    ...
                    var privateRoom = Convert.ToBoolean(reader["privateRoom"]);
                    ...
                    var match = new Match(id)
                    {
                        ...
                        TeamBlueCoins = teamBlueCoins,
                        TeamRed = teamRed,
                        TeamRedPoints = teamRedPoints,
                        TeamRedCoins = teamRedCoins,
                        TeamSize = teamSize,
                        RoomSize = roomSize,
        ...
        public static int AddMatch(Match match)
        {
            ...
            using (var transaction = conn.BeginTransaction())
            {
                    ...
                    "VALUES (@tournament, @mappool, @name, @roomId, @gameMode, @teamMode, @winCondition, @teamBlue, @teamBluePoints, @teamBlueCoins, @teamRed, @teamRedPoints, @teamRedCoins, @teamSize, @roomSize, @rollWinner, @firstPicker, @BO, @viewerMode, @mpTimerCommand, @mpTimerAfterGame, @mpTimerAfterPick, @pointsForSecondBan, @allPicksFreemod, @allPicksNofail, @status, @warmupMode, @matchTime, @privateRoom)", conn))
                {
                    ...
                    command.Parameters.AddWithValue("@teamBlueCoins", match.TeamBlueCoins);
                    command.Parameters.AddWithValue("@teamRedCoins", match.TeamRedCoins);
                    ...
                    command.Parameters.AddWithValue("@allPicksNofail", match.AllPicksNofail);
                    ...
                    command.Parameters.AddWithValue("@privateRoom", match.PrivateRoom);
                    command.ExecuteNonQuery();
                }
                using (var command = new SQLiteCommand("SELECT last_insert_rowid()", conn))
                {
                ...
                {
                    using (var command = new SQLiteCommand("INSERT INTO MatchPicks (match, beatmap, picker, pick, ban, protect, listIndex) VALUES (@match, @beatmap, @picker, True, False, False, @listIndex)", conn))
                    {
                ...
                {
                    using (var command = new SQLiteCommand("INSERT INTO MatchPicks (match, beatmap, picker, pick, ban, protect, listIndex) VALUES (@match, @beatmap, @picker, False, True, False, @listIndex)", conn))
                    {
                        command.Parameters.AddWithValue("@match", match.Id);
                        command.Parameters.AddWithValue("@beatmap", pick.Map.Id);
                        if (match.TeamMode == TeamModes.TeamVS)
                ...
                foreach (var pick in match.Protects)
                {
                    using (var command = new SQLiteCommand(@"INSERT INTO MatchPicks (match, beatmap, picker, pick, ban, protect, listIndex) VALUES (@match, @beatmap, @picker, False, False, True, @listIndex)
                        ON CONFLICT(match, beatmap) DO UPDATE SET protect = True", conn))
                    {
                        command.Parameters.AddWithValue("@match", match.Id);
                        command.Parameters.AddWithValue("@beatmap", pick.Map.Id);
                        if (match.TeamMode == TeamModes.TeamVS)
        ...
        public static void UpdateMatch(Match match)
        {
            ...
            using (var transaction = conn.BeginTransaction())
            {
                ...
                SET tournament = @tournament, mappool = @mappool, name = @name, roomId = @roomId, gameMode = @gameMode, teamMode = @teamMode, winCondition = @winCondition, teamBlue = @teamBlue, teamBluePoints = @teamBluePoints, teamBlueCoins = @teamBlueCoins,
                teamRed = @teamRed, teamRedPoints = @teamRedPoints, teamRedCoins = @teamRedCoins, teamSize = @teamSize, roomSize = @roomSize, rollWinner = @rollWinner, firstPicker = @firstPicker, BO = @BO,
        ...
        public static async Task InitMatchPicks()
        {
            ...
            using (var conn = GetConnection())
            using (var command = new SQLiteCommand("SELECT match, beatmap, picker, pick, ban, protect, listIndex FROM MatchPicks ORDER BY listIndex ASC", conn))
            using (var reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    ...
                    var pick = Convert.ToBoolean(reader["pick"]);
                    var ban = Convert.ToBoolean(reader["ban"]);
                    var protect = Convert.ToBoolean(reader["protect"]);
                    int listIndex;
                    ...
                    if (ban)
                    {
                        if (matchPick.ListIndex == 0)
                            matchPick.ListIndex = match.Bans.Count + 1;
                        match.Bans.Add(matchPick);
                    }
                    if (protect)
                    {
                        if (matchPick.ListIndex == 0)
                            matchPick.ListIndex = match.Protects.Count + 1;
                        match.Protects.Add(matchPick);
                    }
                    if (pick)
                    {
                        if (matchPick.ListIndex == 0)
                            matchPick.ListIndex = match.Picks.Count + 1;
                        match.Picks.Add(matchPick);
                    }
                }
                conn.Close();
            }
